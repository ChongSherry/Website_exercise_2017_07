...
import {
    ...
    Table, Icon, Layout, Modal, message, Switch
} from 'antd';

import ajax from '../base/ajax';
import Modal_add from './channel/modal_channel_add';
import Modal_edit from './channel/modal_channel_edit';
...
class Channel extends Component {
    ...
    // 直接更新显示
    showBool(e, record) {
        ajax.post(ajax.url(ajax.ports.channel.showBool),
            {
                _id: record._id,
                show: e
            }
    ...
    render() {
        ...
        const columns = [{
            ...
            key: 'show',
            render: (v, record) => {
                return (<Switch checkedChildren="是" unCheckedChildren="否" checked={v} onChange={(e) => { me.showBool(e, record) }} />)
            }
        }
            , {
            title: '操作',
...
export default connect(state => ({ state: state.get("view") }))(Channel)